...
| **Airbnb — un −10 % SIN línea de promoción** en reservas a 30–160 d: 9 de 13 desde el 05/08, los 4 anuncios (4 pagaron el 100 %) | **NO IDENTIFICADO** — ¿anticipada configurada como *descuento* a 1 mes? ¿"valoraciones excelentes" activo? No es markup de Guesty (`pms_markup_exists` false) | 05/09/2026 (dato, no configuración) | `reservations` × `pricelabs_fotos`; lo identifica el desglose de `HMR5SYFTDZ` en Airbnb |
...
| Cuándo | Qué |
|---|---|
| **02/09/2026 — EN CURSO por Inbox** | **Exención de comisión de Booking** (`BC-jg7mnkyGW` / nº 5425115874, 206,64 €). El botón prometido para las 24–48 h de marcar la tarjeta como NO VÁLIDA (30/08) **NUNCA apareció** (verificado por Stag el 02/09, +72 h) → el reclamo pasó a **mensaje escrito por el Inbox de la extranet** (redactado el 02/09 en sesión; lo envía Stag), pidiendo exención + confirmación escrita de comisión 0 en la factura de septiembre + nº de caso. **Si en 3–4 días hábiles no contestan: llamar citando el mensaje del Inbox.** Los dos recordatorios por mail (`trig_01J1R5d6…` y `trig_01UyEAUZ…`) ya dispararon y quedan obsoletos. Detalle: CASUISTICAS §5.4 punto 2-bis |
...
| ~~05/09/2026~~ **HECHO 05/09 12:10** | **Marechal, post-cancelación `HMY5R38DJM`**: el refresh de 07:35 UTC registró la cancelación, el 20/10 salió del modo huérfano solo (212 €, min-stay 3, vendible) y el bloque bajó 3–5 % (224/240/257/256/214/211), en banda. **23→26/10 REVENDIDAS a las 10:04 UTC** (`HMR5SYFTDZ`, 654,3 € + 60, payout 580,33 €: el 97 % de lo que pagaba el cancelado por esas noches). Octubre 22/31. Detalle: BITACORA 05/09, Resultado |
| **20/09/2026** | **Marechal, T−30 de lo que queda del hueco**: **20–22/10** (mar–jue, 3 noches; min-stay 3 = el hueco exacto, solo cabe 20→23) y **26/10→04/11** (10 noches). Pace contra el barrio (`pricelabs_mercado_fotos` + `occupancy.daily` de `get_neighbourhood_data`). Si 20–22 sigue sin vender: **min-stay 2** (OK de Stag; riesgo: huérfano de 1 noche, invendible acá). Precio solo si el barrio pasa del ~75 % sin pickup nuestro (§4.8). Antes, no: el barrio compra esas noches entre el ~17/09 y el ~20/10 |
...
| **Pendiente de Stag (esta semana)** | **Identificar el −10 % silencioso de Airbnb**: abrir la reserva `HMR5SYFTDZ` (Marechal 23→26/10) en Airbnb y leer el desglose del precio: el nombre del descuento que aplicó. Después, Precios → Descuentos en los 4 anuncios. Dato: 9 de 13 reservas de Airbnb desde el 05/08 con 30–160 d de antelación pagaron el 90 % del publicado SIN línea de promoción, en los 4 pisos (BITACORA 05/09, Resultado). Hasta saberlo, §3 lo marca NO VERIFICADO y el huésped a 1–3 meses puede estar viendo 0,9× el publicado |
